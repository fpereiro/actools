...
H.ago = function (t) {
   ...
   if (ms < 1000 * 60)           return Math.floor (ms / 1000) + ' seconds ago';
   if (ms < 1000 * 60 * 60)      return Math.floor (ms / (1000 * 60)) + ' minutes ago';
   if (ms < 1000 * 60 * 60 * 24) return Math.floor (ms / (1000 * 60 * 60)) + ' hours ago';
...
H.shorten = function (string, length) {
   var length = length || 1000;
   if (string.length < length) return string;
   return string.slice (0, Math.ceil (length / 2)) + ' [' + (string.length - length) + ' CHARACTERS OMITTED' + '] ' + string.slice (- Math.ceil (length / 2));
}
...
B.mrespond ([
   ...
   ['goto', 'page', function (x, page) {
      ...
      document.title = ['ac;log', page].join (' - ');
   ...
   ['change', ['State', 'page'], {match: B.changeResponder}, function (x) {
      if (B.get ('State', 'page') !== 'dashboard') return;
      if (! B.get ('State', 'query'))   B.call (x, 'set', ['State', 'query'], {filter: [], limit: 100, sort: {field: 't', reverse: true}});
      if (! B.get ('Data', 'logs'))     B.call (x, 'retrieve', 'logs');
      if (! B.get ('Data', 'queries'))  B.call (x, 'retrieve', 'queries');
   }],
   ...
   ['retrieve', 'logs', function (x) {
      ...
      var t = Date.now ();
      B.call (x, 'snackbar', 'yellow', 'Loading...');
      B.call (x, 'post', 'dataout', {}, teishi.copy (B.get ('State', 'query')), function (x, error, rs) {
         if (error) return B.call (x, 'snackbar', 'red', 'There was an error retrieving logs.');
         B.call (x, 'set', ['Data', 'logs'], rs.body);
         B.call (x, 'snackbar', 'green', rs.body.length + ' records retrieved (' + (Date.now () - t) + 'ms)');
      });
   }],
   ['retrieve', 'queries', function (x) {
      B.call (x, 'get', 'queries', {}, '', function (x, error, rs) {
         if (error) return B.call (x, 'snackbar', 'red', 'There was an error retrieving queries.');
         B.call (x, 'set', ['Data', 'queries'], rs.body);
      });
   }],
   ['save', 'query', function (x, name) {
      var query = teishi.copy (B.get ('Data', 'queries', name));
      query.filter = teishi.parse (B.get ('State', 'newfilter'));
      query.name   = name;
      B.call (x, 'post', 'query', {}, query, function (x, error) {
         if (error) return B.call (x, 'snackbar', 'red', 'There was an error saving the query.');
         B.call (x, 'rem', 'State', 'edit');
         B.call (x, 'rem', 'State', 'newfilter');
         B.call (x, 'retrieve', 'queries');
      });
   }],
   ['rename', 'query', function (x, oldName) {
      var name  = prompt ('Enter the new name for the query', oldName);
      if (! name) return;
      B.call (x, 'post', 'query/rename', {}, {oldName: oldName, newName: name}, function (error) {
         if (error) {
            if (error.responseText.match ('exists')) return B.call (x, 'snackbar', 'red', 'There already exists a query with that name.');
            return B.call (x, 'snackbar', 'red', 'There was an error saving the query.');
         }
         if (State.query === oldName) B.call (x, 'set', ['State', 'query'], name);
         B.call (x, 'retrieve', 'queries');
      });
   }],
   ['create', 'query', function (x) {
      ...
      if (! name) return;
      B.call (x, 'post', 'query', {}, {name: name, filter: []}, function (x, error) {
         if (error) {
            if (error.responseText.match ('exists')) return B.call (x, 'snackbar', 'red', 'There already exists a query with that name.');
            return B.call (x, 'snackbar', 'red', 'There was an error creating the new query.');
         }
         B.call (x, 'retrieve', 'queries');
      });
   }],
   ['delete', 'query', function (x, name) {
      B.call (x, 'post', 'query/delete', {}, {name: name}, function (x, error) {
         if (error) return B.call (x, 'snackbar', 'red', 'There was an error deleting the query.');
         if (State.query === name) B.call (x, 'rem', 'State', 'query');
         B.call (x, 'retrieve', 'queries');
      });
   }],
]);
...
views.dashboard = function (x) {
   return B.view ([['Data', 'queries'], ['Data', 'logs'], ['State', 'query']], function (queries, logs, query) {
      if (! queries || ! logs || ! query) return ['div', 'Loading, please wait...'];
      var columns = ['#', 't', 'application', 'environment', 'priority', 'type'];
      var ignoredColumns = ['t', 'username'];

      dale.go (logs, function (v, k) {
         dale.go (v, function (v2, k2) {
            if (inc (ignoredColumns, k2)) return;
            if (! inc (columns, k2)) columns.push (k2);
         });
      });
      return ['div', [
         ['style', [
            ['h3', {margin: 0}],
            ['table', {'font-size': CSS.typography.fontSize (3)}],
         ]],
         ['div', {style: style ({'padding-top': 15, position: 'fixed', 'background-color': 'white', width: 1})}, [
            ['h3', {class: 'floatl'}, ['span', {class: 'action', onclick: B.ev ('retrieve', 'logs')}, 'refresh']],
            /*
            B.view (['State', 'query'], function (query) {
               return ['div', [
                  ['select', {class: 'floatl', style: style ({'margin-left': 10}), onchange: B.ev (
                     ['set', ['State', 'query', 'filter']],
                     ['set', ['State', 'newfilter'], JSON.stringify ((queries [query]) || {}.filter, null, '   ')],
                     ['retrieve', 'logs'],
                  )}, [
                     ['option', 'Select one'],
                     dale.go (queries, function (v, k) {
                        return ['option', {value: k, selected: query === k}, k];
                     })],
                  ],
                  ['h3', {class: 'action floatl', style: style ({'margin-left': 20}), onclick: B.ev ('create', 'query')}, 'create'],
                  query === undefined ? [] : [
                     ['h3', {class: 'action floatl', style: style ({'margin-left': 20}), onclick: B.ev ('rename', 'query', query)}, 'rename'],
                     ['h3', {class: 'action floatl', style: style ({'margin-left': 20}), onclick: B.ev ('delete', 'query', query)}, 'delete'],
                  ],
                  H.if (State.edit, [
                     ['textarea', {class: 'floatl', style: style ({'width, height': 200, 'margin-left': 15}), oninput: B.ev ('set', ['State', 'newfilter'])}, State.newfilter],
                     ['h3', ['span', {class: 'action floatl', style: style ({'margin-left': 50}), onclick: B.ev ('save', 'query', query)}, 'save']],
                  ], [
                     query === undefined ? [] : ['h3', ['span', {class: 'action floatl', style: style ({'margin-left': 50}), onclick: B.ev (
                        ['set', ['State', 'edit'], true],
                        // TODO
                        //['set', ['State', 'newfilter'], JSON.stringify (queries [query].filter, null, '   ')],
                     )}, 'edit']],
            ...
            */
            ['h3', {class: 'action floatl', style: style ({'margin-left': 50}), onclick: B.ev ('logout', [])}, 'logout'],
         ]],
         ['br'], ['br'], ['br'],
         ['br'], ['br'], ['br'],
         ['br'], ['br'], ['br'],
         ['table', {class: 'pure-table pure-table-striped pure-table-bordered'}, [
            ['thead', ['tr', dale.go (columns, function (v) {return ['th', {onclick: (function () {
               var currentlySelected = v === B.get ('State', 'query', 'sort', 'field');
               if (currentlySelected) return B.ev ('set', ['State', 'query', 'sort', 'reverse'], ! B.get ('State', 'query', 'sort', 'reverse'));
               return B.ev (['set', ['State', 'query', 'sort', 'field'], v], ['set', ['State', 'query', 'sort' , 'reverse'], true]);
            }) ()}, v]})]],
            dale.go (logs, function (log, k) {
               return ['tr', [
                  dale.go (columns, function (column) {
                     if (column === '#') return ['td', '#' + (k + 1)];
                     if (column === 't') return ['td', [H.ago (log.t), ' ' + new Date (log.t).toString ().replace (/\(.+/, '')]];
                     if (log [column] === undefined) return ['td'];
                     return ['td', H.shorten (teishi.complex (log [column]) ? JSON.stringify (log [column], null, '   ') : log [column] + '')];
                  }),
               ]];
            }),
         ]],
